...
import CustomTable, {
  ...
  CustomTableHeader,
  CustomTableRow,
} from "@/components/workspace/extension/Table";
    ...
    extensions: [
      ...
      FloatingMenu.configure({
        shouldShow: ({ editor, state }) => {
          ...
          const isText =
            editor.isActive("paragraph") || editor.isActive("textStyle");
    ...
    <div className="flex flex-col md:flex-row h-full w-full">
      <div className="flex-1 rounded">
        <Tiptap
          ...
          handleTitle={(title: string) => setPost({ ...post, title })}
        />
      </div>
      <SidebarEditor
        activeBlock={
          activeTab === "post"
            ? { type: { name: "details" } }
            : activeTab === "ai" ? { type: { name: "ai" } }:currentBlock
            ? currentBlock
            : { type: { name: "none" } }
        }
        editor={editor as Editor}
