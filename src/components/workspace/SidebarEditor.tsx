...
function renderView(
  ...
  if (!type)
    return <div className="text-xs text-gray-500">No block selected</div>;

  switch (type) {
    case "details":
      return (
        <div className="space-y-4 min-h-[70vh] p-4">
          {/* Featured Image */}
...
export default function SidebarEditor({ post, onClose, isOpen, mobile }: SidebarEditorProps) {
  ...
  return (
    <aside
  className={`
    w-full md:w-96 md:border-l flex flex-col text-sm
   z-50
    rounded md:rounded-none
    md:static bottom-0 right-0 h-full
...
>
      ...
      <div className="flex border-b pt-2">
        ...
        {isOpen && mobile && (
            <button
          className={`flex p-2 px-4 text-center font-medium`}
          onClick={() => {
            onClose();
          }}
          // style={!currentBlock ? { opacity: 0.5, cursor: "not-allowed" } : {}}
